...
let instance = {
  ...
  tech: ['./images/Tag.png', './images/Tag (1).png', './images/Tag(2).png'],
  src: 'img1',
  url: [
...
const dataName = ['Tonic', 'Multi', 'Facebook', 'Uber'];
let i = 0;
...
items.forEach((item) => {
  ...
  a.href = '#';
  const img = a.appendChild(createElem('img', item.src));
  img.alt = 'Not Found';
  d.appendChild(thumb);
  const d1 = d.appendChild(createElem('div', 'primary-text'));
  const title = d1.appendChild(createElem('h1', 'title'));
  title.textContent = item.title;
  let ul = d1.appendChild(createElem('ul', 'frame-2'));
  let li = createElem('li', 'client');
  const [tit, det1, det2] = item.details;
  li.textContent = tit;
  ul.appendChild(li);
  const ul1 = createElem('ul', 'inside');
  li = createElem('li', 'details');
  li.textContent = det1;
  ul1.appendChild(li);
  li = createElem('li', 'details');
  li.textContent = det2;
  ul1.appendChild(li);
  ul.appendChild(ul1);
  const p = d1.appendChild(createElem('p', 'description'));
  p.textContent = item.info;
  ul = d1.appendChild(createElem('ul', 'tags'));
  ...
  a = d1.appendChild(createElem('a', 'normal-button'));
  a.href = '#showmenu';
    ...
    for (let ind = 0; ind < items.length; ind += 1) {
      if (items[ind].title.includes(check)) {
        ...
        for (let j = 0; j < items[ind].buttons.length; j += 1) {
          btn[0].children[1].children[j].children[0].href =
            items[ind].buttons[0][j];
          btn[0].children[1].children[j].children[0].children[0].src =
            items[ind].buttons[1][j];
        }
      }
    }
  });
